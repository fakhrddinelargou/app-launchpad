...
type Props = {
  setBudget: React.Dispatch<React.SetStateAction<boolean>>;
};

export default function BudgetForm({setBudget } : Props ) {
...
const token = localStorage.getItem("token")
    const postBudger = useMutation({

        mutationFn : (response : Budget) => fetch("http://localhost:3000/api/budgets" , {
            method : "POST",
     headers : {
  ...
  return (
    ...
    >
      ...
      <h2 className="text-2xl font-bold text-gray-800">Budget Entry</h2>
        <IoIosClose size={30}  onClick={() => setBudget(false)} />
        </div>
