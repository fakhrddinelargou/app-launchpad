...
const schema = z.object({
  ...
  password: z
    .string()
    .min(8, "The password field is required")
    .max(30, "The password field is required"),
    ...
    <div className="w-screen bg-[#f1f5f9] h-screen flex items-center justify-center">
      ...
      >
        ...
        <label className="text-[.7rem] flex gap-1 py-2 pb-3 pl-1">
          ...
          />
          <p className={`${errors.acceptTerm ? "text-red-600" : " "}`}>
            I agree all statements in Terms of servise
          </p>
        </label>
        <button
          type="submit"
          disabled={isSubmitting}
          className="cursor-pointer bg-blue-800 transition-colors flex items-center justify-center  text-white py-2 rounded-[.25rem]  hover:bg-blue-900"
        >
          {!isSubmitting ? (
            "save"
          ) : (
